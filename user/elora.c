...
static int16_t add_measure_data(uint8_t Cmd, uint16_t DataId, uint8_t *buf)
{
    ...
    case MD_DATA_ID_READ_FLOW_METER_INFO:
        ...
        temp = device_comps.meter.total_int;
        buf[i++] = (temp >> 24) & 0xFF;
        buf[i++] = (temp >> 16) & 0xFF;
        buf[i++] = (temp >> 8) & 0xFF;
        buf[i++] = temp & 0xFF;

        temp = f_mul(device_comps.meter.total_dec, pwr(3));
        buf[i++] = (temp >> 24) & 0xFF;
        buf[i++] = (temp >> 16) & 0xFF;
        buf[i++] = (temp >> 8) & 0xFF;
        buf[i++] = temp & 0xFF;
        ...
        {
            dot = device_comps.press_cal_param.dot - 3;
        }
        else
        {
            dot = device_comps.press_cal_param.dot;
        }
        datx = f_div(device_comps.current_press, pwr(dot));
        fp = (uint8_t *)&datx;
        buf[i++] = fp[3];
        buf[i++] = fp[2];
        buf[i++] = fp[1];
        ...
        temp = device_comps.meter.total_intN;
        buf[i++] = (temp >> 24) & 0xFF;
        buf[i++] = (temp >> 16) & 0xFF;
        buf[i++] = (temp >> 8) & 0xFF;
        buf[i++] = temp & 0xFF;

        temp = f_mul(device_comps.meter.total_decN, pwr(3));
        buf[i++] = (temp >> 24) & 0xFF;
        buf[i++] = (temp >> 16) & 0xFF;
        buf[i++] = (temp >> 8) & 0xFF;
        buf[i++] = temp & 0xFF;
...
static uint8_t Pro_lora(uint8_t Cmd, uint8_t *buf, uint8_t len)
{
    ...
    if (Cmd == 0) // Read
    {
        ...
        case MD_DATA_ID_READ_FLOW_METER_INFO:  // flow meter info
#if (MD_ELORA_VER_SEL == MD_ELORA_VER_A39C)
            if (buf[0] == 0xff)
            {
                loraMisc.ack_delay_tmr = (uint16_t)loraMisc.cfg_info.nodeId * 8 + 6;
            }
            else
            {
            ...
            if (buf[0] == 0xff)
            {
                loraMisc.ack_delay_tmr = (uint16_t)loraMisc.cfg_info.nodeId * 8 + (MD_AS32_WORK_MODE_RECV_DELAY_MS / 50 + 9 + 3);
            }
            else
            {
