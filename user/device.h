...
#define  MD_LORA                 2
#define  MD_NO_IOT               3
//#define  MD_FLOW_CONTROLER       4
#define  MD_PRODUCT_NAME         MD_WIFI //  must be defined!!!!!
...
//#define   MD_EXT_SCI
#define   MD_PRESS       0
#define   MD_EXT_SCI_TYPE   MD_PRESS
...
#define  BK_LN
  typedef struct temp_comp_param_t
  {
...
typedef struct press_cal_param_t
{
    int32_t x[8];
    int32_t y[8];
...
typedef struct flow_cal_param_t
{
...
typedef struct res_cal_param_t
{
    int32_t x[2];
    int32_t y[2];
    uint8_t dot;
...
typedef struct manufacturer_info_t
{
    uint8_t name[25];
    uint8_t cs;
}manufacturer_info_t;
    
typedef struct coe_t
{
...
typedef union device_trigger_req_t
{
    uint16_t all;
    struct
    {
        uint8_t interval_time :1;
        uint8_t time_auto :1;
        uint8_t press_high_over :1;
        uint8_t press_high_realse :1;
        uint8_t press_low_less :1;
        uint8_t press_low_realse :1;
        uint8_t temp_high_over :1;
        uint8_t temp_high_realse :1;
        uint8_t temp_low_less :1;
        uint8_t temp_low_realse :1;
    }_bit;
}device_trigger_req_t;
...
typedef struct _DEVICE_COMPONENTS
{
    ...
    device_sw_t sw;
    device_trigger_req_t trigger_req;
