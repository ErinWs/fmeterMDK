...
| ---- | ---- | ---- | ---- | ---- |
| 0 | 频率分段点数 | SDXXXXXX |  |  |
| 1 | 频率系数 a | SDXXXXXX |  | $k=af^4+bf^3+cf^2+df+e$ 用于频率计算仪表系数 |
| 2 | 频率系数 b | SDXXXXXX |  |  |
| 3 | 频率系数 c | SDXXXXXX |  |  |
| 4 | 频率系数 d | SDXXXXXX |  |  |
| 5 | 频率系数 e | SDXXXXXX |  |  |
...
- **写请求**:<br>`addr 11H 30 01 00 xx len 2B频率分段数,分段点0对应2B频率值&4B(float)仪表系数…2byte(crc)`
...
| ---- | ---- | ---- | ---- | ---- |
| 0x00 | 温度 | 浮点数 |  | R |
| 0x02 | 瞬时流量 | 浮点数 | 单位参考瞬时流量单位[参考单位](#单位选择表) | R |
| 0x04 | 流速 m/s | 浮点数 |  | R |
| 0X06 | 频率 | 浮点数 |  | R |
| 0X08 | 累计流量整数部分 | 32位整数 |  | R |
| 0X0A | 累计流量小数部分 | 32位整数 | 6位定点小数累计流量＝小数部分 * 0.000001；累计单位：m³ | R |
| 0X0C | 瞬时流量单位 | 浮点数 | <a id="单位选择表"> (0: m³/h, 1: L/min, 2: Kg/h, 3: L/h, 4: T/h, 5: Kg/min, 6: m³/min, 7: t/min) </a> | R |
| 0X0E | 读取压力 | 浮点数 | 单位 Kpa | R |
| 0x10 | 读取标况流量 | 浮点数 | Nm³/h | R |
| 0x12 | 读取累计标况整数 | 32位整数 | 单位：Nm³ | R |
| 0x14 | 读取累计标况小数 | 32位整数 | 6位定点小数累计流量＝小数部分 * 0.000001；单位：Nm³ | R |
...
|--- | ----|以下16位寄存器|----|----|
| 0X2d | 触发理论4/20ma交替输出时长值 | 16位整数 | 间隔4秒交替，单位为秒，最长60秒 | WR |
| 0X2e | 实际4ma输出值 | 16位整数 | 单位0.001ma | WR |
| 0X2f | 实际20ma输出值 | 16位整数 | 单位0.001ma | WR |
...
| 字段名 | 数据来源 | 说明 |
|--------|----------|------|
| `$%0.20s$` | `device_comps.manufacturer_info.name` | 厂商信息，用$符号包围 |
| `TYPE:%s` | `device_comps.device_info.type` | 设备类型 |
| `ID:%s` | `device_comps.device_addr.addr` | 设备地址(ID) |
| `LNG:%d.%05d` | `device_comps.gps.glng` | 经度信息 |
| `LAT:%d.%05d` | `device_comps.gps.glat` | 纬度信息 |
| `ICCID:%s` | `netComps.net_info.iccid` | ICCID号码 |
| `VBATT:%d.%dV` | `device_comps.batt` | 电池电压 |
| `CSQ:%d` | `netComps.net_info.Csq` | 信号强度 |
| `WK_FLOW:%f` | `device_comps.current_flow` | 工况流量 |
| `TOTAL_FLOW:%d.%06d` | `device_comps.meter.total_int`, `device_comps.meter.total_dec` | 累计工况流量 |
| `TEMP:%d.%d` | `device_comps.current_temp` | 温度 |
| `PFS:%fKPa` 或 `PFS:NOCAL` | `device_comps.press_full_scale` | 压力满量程(已标定)或未标定 |
| `PRES:%fKPa` 或 `PRES:NOCAL` | `device_comps.current_press` | 当前压力(已标定)或未标定 |
| `STD_FLOW:%f` | `device_comps.current_flowN` | 标况流量 |
| `TOTAL_FLOWN:%d.%06d` | `device_comps.meter.total_intN`, `device_comps.meter.total_decN` | 累计标况流量 |
| `TIME:%s` | `device_comps.system_time.time` | 时间戳 |
| `EVENT:%d` | `event`参数 | 事件编号 |
| `END` | 固定值 | 结束标识 |
...
| 瞬时流量 | 4 | 4字节浮点数 | m³/h  |
| 累计流量整数 | 4 | 32位整数 | m³ |
| 累计流量小数部分 | 4 | 32位整数，6位定点小数，小数部分*0.000001| m³ |
| 当前压力 | 4 | 32位浮点数 | m³ |
| 标况瞬时流量 | 4 | 32位浮点数 |Kpa |
| 标况累计整数 | 4 | 32位整数| Nm³ |
| 标况累计小数 | 4 | 32整数，6位定点小数，小数部分*0.000001|Nm³ |
| 当前温度 | 2 | 16位整数 |0.1℃ |
| batt | 1 | 8位整数|0.1V  |
| rssi | 1 | 0-255 |-164+rssi dbm |
